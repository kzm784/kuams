...
class KuamsTeleop : public rclcpp::Node
{
public:
    KuamsTeleop() : Node("kuams_teleop")
    {
        ...
        joy_sub_ = this->create_subscription<sensor_msgs::msg::Joy>("joy", 10, std::bind(&KuamsTeleop::joyCallback, this, std::placeholders::_1));

        // Create a timer to publish at regular intervals
        timer_ = this->create_wall_timer(std::chrono::milliseconds(100), std::bind(&KuamsTeleop::publish, this));
    }
...
int main(int argc, char **argv)
{
    ...
    auto node = std::make_shared<KuamsTeleop>();
    rclcpp::spin(node);
    rclcpp::shutdown();
    return 0;
}
